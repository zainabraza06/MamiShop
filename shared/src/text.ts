/**
 * Pure text, date and URL helpers shared by the backend and frontend.
 *
 * Moved out of the app's `lib/utils.ts`. The Tailwind class merger `cn` stayed
 * behind in the frontend, because it depends on UI tooling that has no business
 * in a package the backend also imports.
 */
...
/** Turns Date objects into JSON-safe values before crossing a serialisation boundary. */
...
/** Drops null/undefined entries so partial updates stay clean. */
