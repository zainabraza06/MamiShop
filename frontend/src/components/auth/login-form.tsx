 ...
 *
 * The exception is a suspended account, which the proxy reports explicitly:
 * that user needs to know to contact support rather than keep retrying a
 * password that is, in fact, correct.
 *
 * The rest arrive as an `error` code in the query string when Google sign-in
...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  OAuthCallbackError: 'Google sign-in did not finish. Please try again.',
    ...
    <div className="space-y-4">
      ...
      <Button
        ...
        onClick={() => {
          ...
          // has to leave the page. The API owns the flow and sends the browser
          // back to callbackUrl once it completes.
          //
          // Not a Next.js route: /api is proxied to the API, which answers with
          // a redirect to Google. router.push() would try to navigate to it as
          // a page and find nothing to render.
          // eslint-disable-next-line @next/next/no-location-assign-relative-destination
          window.location.assign(`/api/auth/google?callbackUrl=${encodeURIComponent(callbackUrl)}`);
        }}
      >
        <GoogleIcon />
