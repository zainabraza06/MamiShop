...
async function seedCategories() {
  ...
  for (const [index, parent] of tree.entries()) {
    // Tile artwork lives in public/categories, generated by npm run gen:images.
    const categoryImage = `/categories/${parent.slug}.webp`;

    const row = await prisma.category.upsert({
      where: { slug: parent.slug },
      update: { sizingTemplate: parent.sizingTemplate, isActive: true, imageUrl: categoryImage },
      create: {
        name: parent.name,
        slug: parent.slug,
        position: index,
...
async function seedProducts(categoryIds: Map<string, string>) {
  ...
  for (const [index, product] of PRODUCTS.entries()) {
    ...
    for (const [variantIndex, variant] of product.variants.entries()) {
      ...
      variantIdBySku.set(variant.sku, variantRow.id);
    }

    /**
     * Images are replaced rather than only created when missing.
     *
     * The original seed skipped this block whenever any image already existed,
     * which meant a database seeded before the artwork changed kept its old
     * (now dead) URLs forever. Deleting and recreating keeps the seed
     * authoritative for its own demo data and still idempotent — the end state
     * is identical however many times it runs.
     */
    ...
    await prisma.productImage.createMany({
      data: [
        {
          productId: row.id,
          url: `/products/${product.sku}-1.webp`,
        ...
        {
          productId: row.id,
          url: `/products/${product.sku}-2.webp`,
          alt: `Close detail of the ${product.fabric.toLowerCase()} used for ${product.name}`,
          position: 1,
        },
        // One per colour, tied to its variant so choosing a colour swaps the
        // gallery rather than leaving the same photo under a new label.
        ...product.variants.map((variant, variantIndex) => ({
          productId: row.id,
          variantId: variantIdBySku.get(variant.sku) ?? null,
          url: `/products/${variant.sku}.webp`,
          alt: `${product.name} in ${variant.name.toLowerCase()}`,
          position: 2 + variantIndex,
        })),
      ],
    });
