...
import { priceOrder, loyaltyPointsEarned, loyaltyPointsToMinor } from '@momishop/shared/pricing';
import { evaluateCoupon, normalizeCouponCode } from '@momishop/shared/coupons';
import { resolveShippingZone, resolveTaxRules, availableRates } from '@momishop/shared/shipping';
import { CACHE_KEYS, CACHE_TTL, cached } from '../lib/cache';
import { formatOrderNumber } from '@momishop/shared/text';
...
import type { CheckoutInput } from '@momishop/shared/validation';
...
/**
 * Runs the order transaction, retrying on an order-number collision.
 *
 * `nextOrderNumber()` derives the number from a COUNT inside the transaction,
 * so two checkouts committing at the same moment can compute the same value.
 * The unique constraint on `Order.orderNumber` is what actually guarantees
 * uniqueness; this is the loser's retry.
 *
 * Without it the race is rare but real, and it surfaces as one customer seeing
 * a checkout error at exactly the busiest moment. It was first observed as a
 * flaky end-to-end test running four workers against one database — which is a
 * far cheaper place to find it than production.
 *
 * Only P2002 on `orderNumber` is retried. Anything else (out of stock,
...
export async function runOrderTransaction(
  ...
  for (let attempt = 1; ; attempt++) {
    try {
      ...
      if (attempt >= MAX_ATTEMPTS || !isUniqueViolation(error, 'orderNumber')) throw error;

      logger.warn('Order number collided; retrying the transaction', { attempt });

      // A short jittered pause so two colliding requests do not immediately
